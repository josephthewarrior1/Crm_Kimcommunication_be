...
public class ProjectController {
    ...
    public List<Project> listProjects(@RequestHeader(value = "Authorization", required = false) String auth) {
        ...
        if (u == null) return projectRepository.findAll();
        if (isAdminOrManager(u)) return projectRepository.findAll();
    ...
    public ResponseEntity<Project> createProject(@Valid @RequestBody Project project,
                                                 @RequestHeader(value = "Authorization", required = false) String auth) {
        AppUser u = currentUser(auth);
        if (u == null || !isAdminOrManager(u)) return ResponseEntity.status(403).body((Project) null);
        Project saved = projectRepository.save(project);
        return ResponseEntity.created(URI.create("/api/projects/" + saved.getId())).body(saved);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Project> updateProject(@PathVariable Long id, @RequestBody java.util.Map<String, Object> updates,
                                                 @RequestHeader(value = "Authorization", required = false) String auth) {
        ...
        return projectRepository.findById(id)
                .map(existing -> {
                    ...
                    // Merge fields from incoming updates with existing project (partial update support)
                    ...
                    if (updates.containsKey("priority")) {
                        Object priorityObj = updates.get("priority");
                        if (priorityObj != null) {
                            try {
                                existing.setPriority(Priority.valueOf(priorityObj.toString().toUpperCase()));
                            } catch (IllegalArgumentException e) {
                    ...
                    if (updates.containsKey("endDate") || updates.containsKey("eventDate")) {
                        Object dateObj = updates.containsKey("endDate") ? updates.get("endDate") : updates.get("eventDate");
                        if (dateObj != null && !dateObj.toString().trim().isEmpty()) {
                            try {
                                ...
                                if (dateObj instanceof java.time.LocalDate) {
                                    ...
                                    System.out.println("Updated endDate from string: " + dateStr + " -> " + existing.getEndDate());
                                }
                            } catch (Exception e) {
                    ...
                    // Note: We don't update relationships (clientEntity, stages, events, etc.) from the request
    ...
    public ResponseEntity<Void> deleteProject(@PathVariable Long id,
                                              @RequestHeader(value = "Authorization", required = false) String auth) {
        AppUser u = currentUser(auth);
        if (u == null || !isAdminOrManager(u)) return ResponseEntity.status(403).<Void>build();
        if (!projectRepository.existsById(id)) return ResponseEntity.status(404).<Void>build();
    ...
    public ResponseEntity<List<WorkflowStage>> listStages(@PathVariable Long projectId,
        ...
        if (opt.isEmpty()) return ResponseEntity.status(404).<List<WorkflowStage>>build();
        AppUser u = currentUser(auth);
        if (u != null && !isAdminOrManager(u) && !hasAccess(opt.get(), u)) return ResponseEntity.status(403).body((java.util.List<com.pms.domain.WorkflowStage>) null);
    ...
    public ResponseEntity<WorkflowStage> addStage(@PathVariable Long projectId, @Valid @RequestBody WorkflowStage stage,
                                                  @RequestHeader(value = "Authorization", required = false) String auth) {
        Optional<Project> opt = projectRepository.findById(projectId);
        if (opt.isEmpty()) return ResponseEntity.status(404).<WorkflowStage>build();
        AppUser u = currentUser(auth);
        if (u != null && !isAdminOrManager(u) && !hasAccess(opt.get(), u)) return ResponseEntity.status(403).body((WorkflowStage) null);
        stage.setId(null);
        stage.setProject(opt.get());
        WorkflowStage saved = workflowStageRepository.save(stage);
        return ResponseEntity.created(URI.create("/api/projects/" + projectId + "/stages/" + saved.getId())).body(saved);
    }
    ...
    public ResponseEntity<List<ProjectEvent>> listEvents(@PathVariable Long projectId,
                                                        @RequestHeader(value = "Authorization", required = false) String auth) {
        Optional<Project> opt = projectRepository.findById(projectId);
        if (opt.isEmpty()) return ResponseEntity.status(404).<List<ProjectEvent>>build();
        AppUser u = currentUser(auth);
        if (u != null && !isAdminOrManager(u) && !hasAccess(opt.get(), u)) return ResponseEntity.status(403).body((java.util.List<com.pms.domain.ProjectEvent>) null);
    ...
    public ResponseEntity<ProjectEvent> addEvent(@PathVariable Long projectId, @Valid @RequestBody ProjectEvent event,
                                                 @RequestHeader(value = "Authorization", required = false) String auth) {
        Optional<Project> opt = projectRepository.findById(projectId);
        if (opt.isEmpty()) return ResponseEntity.status(404).<ProjectEvent>build();
        AppUser u = currentUser(auth);
        if (u != null && !isAdminOrManager(u) && !hasAccess(opt.get(), u)) return ResponseEntity.status(403).body((ProjectEvent) null);
        event.setId(null);
        event.setProject(opt.get());
        ProjectEvent saved = projectEventRepository.save(event);
        return ResponseEntity.created(URI.create("/api/projects/" + projectId + "/events/" + saved.getId())).body(saved);
    }
    ...
    public ResponseEntity<List<ProjectDocument>> listDocuments(@PathVariable Long projectId,
                                                               @RequestHeader(value = "Authorization", required = false) String auth) {
        Optional<Project> opt = projectRepository.findById(projectId);
        if (opt.isEmpty()) return ResponseEntity.status(404).<List<ProjectDocument>>build();
        AppUser u = currentUser(auth);
        if (u != null && !isAdminOrManager(u) && !hasAccess(opt.get(), u)) return ResponseEntity.status(403).body((java.util.List<com.pms.domain.ProjectDocument>) null);
        return ResponseEntity.ok(projectDocumentRepository.findByProjectId(projectId));
    }

    @PostMapping("/{projectId}/documents")
    public ResponseEntity<ProjectDocument> addDocument(@PathVariable Long projectId, @Valid @RequestBody ProjectDocument doc,
                                                       @RequestHeader(value = "Authorization", required = false) String auth) {
        Optional<Project> opt = projectRepository.findById(projectId);
        if (opt.isEmpty()) return ResponseEntity.status(404).<ProjectDocument>build();
        AppUser u = currentUser(auth);
        ...
        return ResponseEntity.created(URI.create("/api/projects/" + projectId + "/documents/" + saved.getId())).body(saved);
    }
    ...
    public ResponseEntity<List<Contact>> listContacts(@PathVariable Long projectId,
                                                      @RequestHeader(value = "Authorization", required = false) String auth) {
        Optional<Project> opt = projectRepository.findById(projectId);
        if (opt.isEmpty()) return ResponseEntity.status(404).<List<Contact>>build();
        AppUser u = currentUser(auth);
        if (u != null && !isAdminOrManager(u) && !hasAccess(opt.get(), u)) return ResponseEntity.status(403).body((java.util.List<com.pms.domain.Contact>) null);
    ...
    public ResponseEntity<Contact> addContact(@PathVariable Long projectId, @Valid @RequestBody Contact contact,
                                              @RequestHeader(value = "Authorization", required = false) String auth) {
        Optional<Project> opt = projectRepository.findById(projectId);
        if (opt.isEmpty()) return ResponseEntity.status(404).<Contact>build();
        AppUser u = currentUser(auth);
        ...
        return ResponseEntity.created(URI.create("/api/projects/" + projectId + "/contacts/" + saved.getId())).body(saved);
    }
    ...
    public ResponseEntity<Void> assignTeamMember(@PathVariable Long projectId, @PathVariable Long memberId,
        ...
        try {
            ...
            System.out.println("Assigning team member " + m.getId() + " (" + m.getName() + ") to project " + p.getId() + " (" + p.getName() + ")");
    ...
    public ResponseEntity<Void> unassignTeamMember(@PathVariable Long projectId, @PathVariable Long memberId,
        ...
        Optional<Project> optP = projectRepository.findById(projectId);
        Optional<TeamMember> optM = teamMemberRepository.findById(memberId);
        if (optP.isEmpty() || optM.isEmpty()) return ResponseEntity.status(404).<Void>build();
        Project p = optP.get();
        TeamMember m = optM.get();
    ...
    public ResponseEntity<List<java.util.Map<String,Object>>> listProjectUsers(@PathVariable Long projectId,
        ...
        return projectRepository.findById(projectId)
                .map(p -> {
                    AppUser u = currentUser(auth);
                    if (u != null && !isAdminOrManager(u) && !hasAccess(p, u)) return ResponseEntity.status(403).body((java.util.List<java.util.Map<String,Object>>) null);
    ...
    public ResponseEntity<Void> assignProjectUser(@PathVariable Long projectId, @PathVariable Long userId,
        ...
        if (optP.isEmpty() || optU.isEmpty()) return ResponseEntity.status(404).<Void>build();
        Project p = optP.get();
    ...
    private boolean hasAccess(Project p, AppUser u) {
        ...
        if (p.getUsers() != null && u != null) {
            Long uid = u.getId();
            if (uid != null && p.getUsers().stream().anyMatch(x -> uid.equals(x.getId()))) return true;
        }
        // fallback: by email matching in team members
